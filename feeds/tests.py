...
works.
    ...
    def test_consolidated_feed(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], models.TwitterStatus.objects.filter(followed_from__uuid=auth_token.uuid).count())
    ...
    def test_link_timestamps(self):
        """Tests to make sure timestamps getting stored are correct"""
